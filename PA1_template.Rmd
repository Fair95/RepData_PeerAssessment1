...
```
The mean is `tmean`, the median is `tmedian`.
## What is the average daily activity pattern?
```{r, echo = TRUE}
is <- aggregate(steps~interval, dat, mean, rm.na = TRUE)
ggplot(data = is, aes(x = interval, y = steps)) + geom_line()
```
The interval contains the maximum number of steps is [`is$interval[which.max(is$step)]`, `is$interval[which.max(is$step)]+5`]
...
```
It appears that we have `nmissing` missing values.
...
```
The new mean is `tmean2` and new median is `tmedian2`
